...
window.addEventListener("DOMContentLoaded", function (event) {
   ...
   const name = this.localStorage.getItem('name');
    ...
    document.getElementById('easy').addEventListener('click',function(){
        ...
        Toastify({
            text: "Minus Press space pour commencer!",
            duration: 3000,
            ...
            style: {
              background: "linear-gradient(to bottom, #000000, #96c93d)",
            },
            onClick: function(){} // Callback after click
          }).showToast();
        noteElement.style.opacity = 1;
        resetGame();
        document.getElementById('menus_contenaire').style.display = 'none';
    })
    document.getElementById('hard').addEventListener('click',function(){
        ...
        speed = 50;
        Toastify({
            text: "Boss Press space pour commencer!",
            duration: 3000,
            ...
            stopOnFocus: true, // Prevents dismissing of toast on hover
            style: {
              background: "linear-gradient(to bottom, #000000, red)",
            },
            onClick: function(){} // Callback after click
          }).showToast();
        noteElement.innerHTML = `Press space pour commencer!`;
        noteElement.style.opacity = 1;
        resetGame();
        document.getElementById('menus_contenaire').style.display = 'none';
    })
    ...
    window.addEventListener("keydown", function (event) {
      ...
      if (event.key == "H" || event.key == "h") {
        ...
        speed = 50;
        fadeExponential = 1.025;
        noteElement.innerHTML = `Extreme . Press space!`;
        noteElement.style.opacity = 1;
        resetGame();
      ...
      if (event.key == "E" || event.key == "e") {
        ...
        speed = 150;
        fadeExponential = 1.024;
        noteElement.innerHTML = `Leger. Press space!`;
        noteElement.style.opacity = 1;
        resetGame();
    ...
    function main(timestamp) {
      try {
        ...
        Toastify({
            ...
            stopOnFocus: true, // Prevents dismissing of toast on hover
            style: {
              background: "linear-gradient(to top, #000000, red)",
            },
            onClick: function(){} // Callback after click
          }).showToast();
      }
    ...
    function stepAndTransition(percentageOfStep) {
      ...
      {  setTile(head, {
        ...
        grid.style.transform = "perspective(800px) rotateX(20deg)";}
